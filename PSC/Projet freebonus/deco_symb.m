function suite_bits=deco_symb(suite_symb,M)

% La fonction deco_symb permet de décoder une suite de symboles pour
% obtenir la suite binaire associée.

% suite_symb est la suite des symboles sous forme décimale.
...
longs=length(suite_symb); % Nombre de symboles en entrée
...
for i=1:longs % On parcourt la suite de symboles
    b=dec2bin(suite_symb(i),k); % Conversion des symboles en binaire
