...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 y1= zeros(1,Nx+M-1);    % initialisation du signal de sortie total
 ...
 %[msg,x]=generation_tableau_trames('',Nx,1,0.005); 
   ...
   while i < Nx
       il = min(i+L-1,Nx);   
       X=fft(i:il),N);
       yt = ifft( X .* H, N);       % trame de sortie de taille N donc debordement sur la trame suivante
       k  = i + N - 1;
       y1(i:k) = y1(i:k) + yt;         % addition des blocs de sortie qui se superposent sur M-1 valeurs
       i = i+L;
   end
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   while i < Nx
       ...
       yt2 = conv(xt2,h);
       k  = i + (L+M-1) - 1;
       y2(i:k) = y2(i:k) + yt2;   % (add the overlapped output blocks)
       i = i+L;
   end
   figure(2)
   subplot(3,1,1) ; plot(x);  xlim([xmin xmax]);            title('Input Signal x');
